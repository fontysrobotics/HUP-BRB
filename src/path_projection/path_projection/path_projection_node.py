...
class PathProjection(Node):
    ...
    def robot_info_callback(self, msg: RobotInformation):
        ...
        if not robot.id in self.robots:
            path_subscription = self.create_subscription(Path, robot.id+'/plan', lambda msg: self.save_plan(robot, msg), 10)
            location_subscription = self.create_subscription(Odometry, robot.id+'/odom', self.location_callback, 10)
            self.robots[robot.id] = {"info": robot, "path_subscription": path_subscription, "location_subscription": location_subscription}   
    

    def save_plan(self, robot: RobotInformation, msg: Path):
        self.robots[robot.id]["path"] = msg
        self.draw_line(msg)

              


    def draw_line(self, msg: Path):
        window_x = 1280
        window_y = 1024
